...
public class Df_Geoffrus : DialogueFlow
{
    ...
    public override async Task StartDialogue()
    {
        ...
        if (_firstTalk)
        {
            ...
            if (result == 0)
            {
                await _characters.Bobbus.Say("Vous êtes un genre de démone ?");
                await _characters.Geoffrrus.Say("C'est exact. Une démone marchande pour être plus précise.");
                await _characters.Bobbus.Say("Marchande... Marchande de quoi exactement ?");
            ...
            if (result == 1)
            {
                ...
                await _characters.Geoffrrus.Say("A moi, oui. Je suis l'unique marchande de cet endroit.");

...
public class Df_Tuto : DialogueFlow
{
    ...
    public override async Task StartDialogue()
    {
        ...
        await _characters.Charon.Say("C’est bon, vous êtes remis de vos émotions ? Je vous ai trouvé sur la rive du fleuve il y'a déjà plusieurs heures.");
       ...
       // await _characters.Bobbus.Say("….");
        ...
        //await _characters.Charon.Say("Oh, c’est très simple ! Disons qu’au vu de votre vie… mouvementée, on vous laisse une chance de vous racheter.");
        await _characters.Charon.Say("... Disons qu’après votre vie… mouvementée, on vous laisse parfois une chance de vous racheter.");
        //await _characters.Charon.Say("Racheter votre âme, plus précisément. Vous voyez ce sac ? Il contient un paquet d’âmes comme la vôtre, encore pleines des pêchés de leur vie passée,");
        //await _characters.Charon.Say("mais ne demandant qu’à accéder à la félicité éternelle du paradis… Et il vous incombe de vous en occuper");
        ...
        await _characters.Charon.Say("Votre nouvelle mission, mon jeune ami,sera de vous occuper ici d'ames comme la votre, et de les purifier chaque jour. Vous devriez vous sentir honoré, ça n’est pas donné à tout le monde !");
        ...
        await _characters.Charon.Say("Vous allez voir, c’est enfantin ! Les âmes sont sous formes de petites graines, qu’ils vous suffit de planter dans le sol des parcelles après les avoir bêchées.");
        await _characters.Charon.Say("Arrosez les, et après quelques jours vous obtiendrez une belle fleur d’âme toute pure que vous n’aurez qu’à récolter avec une faux et apporter au moulin pour finir l’opération.");
        await _characters.Charon.Say("Faite cependant attention, la corruption a tendance à se… répandre de manière fâcheuse entre les pousses, et vous risquez de rapidement vous retrouver avec un champ pourri si vous n'en prenez bas bien soin.");

        await _characters.Bobbus.Say("Et une fois que tout est fini ?");

        await _characters.Charon.Say("Vous recommencez, quelle question. Je reviendrais vous livrez des sacs ici même tous les jours, jusqu’à ce que vous ayez ammassé assez d'argent pour me racheter votre âme à votre tour.");
        ...
        await _characters.Charon.Say("En cas de doute,regardez dans votre maison, il y a un manuel qui résume tout ce que je viens de vous explique.Votre prédécesseur a eu la bonne idée de compiler plusieurs de ses observations dans un herbier. Bonne chance!");

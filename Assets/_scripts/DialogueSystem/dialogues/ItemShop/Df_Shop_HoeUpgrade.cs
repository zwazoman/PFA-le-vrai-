...
public class Df_Shop_HoeUpgrade : DialogueFlow
{
    ...
    public override async Task StartDialogue()
    {
        ...
        if (resultat == 0)
        {
            ...
            {
                await _characters.Geoffrrus.Say("ça c'est un client comme je les aime!");
                WorldObject.SendMessage("SellItem");
            }
            else
            {
                await _characters.Geoffrrus.Say("Retourne travailler,clochard");
            }
        }
        else
        {
            ...
            if (resultat2 == 0)
            {
                ...
                {
                    await _characters.Geoffrrus.Say("Je veux bien faire un geste, mais t'es vraiment trop pauvre. Retourne travailler,clochard");
                }
            }
            else
            {

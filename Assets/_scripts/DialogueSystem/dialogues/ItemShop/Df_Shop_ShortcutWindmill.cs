...
public class Df_Shop_ShortcutWindmill : DialogueFlow
{
    ...
    public override async Task StartDialogue()
    {
        ...
        await _characters.Geoffrrus.Say("Ce que cette #clef# ouvre ? Je n’en ai aucune idée.");
        ...
        await _characters.Geoffrrus.Say($"Que dis tu de #{((SellingSpot)WorldObject).price} poussières d'âmes# pour cela ?");

        int resultat = await _characters.Narrator.Ask($"Voulez vous acheter cette #clef# pour #{((SellingSpot)WorldObject).price} poussières d'âmes# ? (Vous en avez #{PlayerMain.Instance.Stats.Money}#)", new string[] { "J'achète !", "J'ai changé d'avis." });
        if (resultat == 0)
        {
